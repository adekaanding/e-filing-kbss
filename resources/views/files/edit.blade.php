    ...
    <form action="{{ route('files.update', $file->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="reference_no" id="reference_no" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value="{{ old('reference_no', $file->reference_no) }}" required>
                @error('reference_no')
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" id="title" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" value="{{ old('title', $file->title) }}" required>
                @error('title')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" class="mt-1 focus:ring-primary focus:border-primary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                    ...
                    <option value="{{ $key }}" {{ old('status', $file->status) == $key ? 'selected' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
                @error('status')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
